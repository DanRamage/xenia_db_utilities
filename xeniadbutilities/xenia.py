...
class xeniaDB:
    ...
    def getErrorInfo(self):
        ...
        if (self.lastErrorFile != None and self.lastErrorLineNo != None and self.lastErrorFunc != None):
            errorMsg += " Function: %s Line: %s File: %s" % (
            self.lastErrorFunc, self.lastErrorLineNo, self.lastErrorFile)
        return (errorMsg)
    ...
    def addSensor(self, obsName, uom, platformHandle, active=1, fixedZ=0, sOrder=1, mTypeID=None, addObsAndUOM=False):
        ...
        if (mTypeID == None):
            ...
            if (scalarID == -1):
                ...
                # Error occured so return.
                if (scalarID == None):
                    self.lastErrorMsg += "\nUnable to add scalar_type with obs_type_id: %d and uom_type_id: %d" % (
                    obsTypeID, uomTypeID)
                    return (None)
            elif (scalarID == None):
                return (None)
    ...
    def updateMeasurement(self, mTypeID, sensorID, platformHandle, date, lat, lon, z, mValues, sOrder=1,
                          autoCommit=True, rowEntryDate=None, updateDate=None):
        ...
        if (dbCursor != None):
            ...
            if (addRec == False):
                ...
                return (
                    self.addMeasurementWithMType(mTypeID, sensorID, platformHandle, date, lat, lon, z, mValues, sOrder,
                                                 autoCommit, rowEntryDate))
    ...
    def addMeasurementWithMType(self, mTypeID, sensorID, platformHandle, date, lat, lon, z, mValues, sOrder=1,
                                autoCommit=True, rowEntryDate=None):
        # DWR 2011-07-25
    ...
    def addMeasurement(self, obsName, uom, platformHandle, date, lat, lon, z, mValues, sOrder=1, autoCommit=True,
        ...
        if (sensorID == -1):
            self.lastErrorMsg = "Unable to add measurement. Sensor: %s(%s) does not exist on platform: %s. No entry in sensor table." % (
            obsName, uom, platformHandle)
            return (None)
        elif (sensorID == None):
            return (None)
        mTypeID = self.getMTypeFromObsName(obsName, uom, platformHandle, sOrder)
        if (mTypeID == -1):
            self.lastErrorMsg = "Unable to add measurement. Sensor: %s(%s) does not exist on platform: %s. No entry in m_type table." % (
            obsName, uom, platformHandle)
            return (None)
        elif (mTypeID == None):
            return (None)
        return (self.addMeasurementWithMType(mTypeID, sensorID, platformHandle, date, lat, lon, z, mValues, sOrder,
                                             autoCommit, rowEntryDate))
...
class xeniaPostGres(xeniaDB):
    ...
    def getObsDataForPlatform(self, platform, lastNHours=None):
        ...
        dateOffset = ''
        if (lastNHours != None):
            dateOffset = "m_date >  date_trunc('hour',( SELECT timezone('UTC', now()-interval '%d hours' ) ) ) AND" % (
                lastNHours)
...
class uomconversionFunctions:
    ...
    def measurementConvert(self, value, fromUOM, toUOM):
        try:
            ...
            value, fromUOM, toUOM, conversionString, traceback.format_exc())
            print(msg)
